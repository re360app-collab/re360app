...
const AdminLeadAssignmentModal = ({ lead, onClose, onLeadAssigned }) => {
  ...
  useEffect(() => {
    const fetchLoanOfficers = async () => {
      ...
      const { data, error } = await supabase
        ...
        .in('position', ['Loan Officer', 'NMB Admin']); // NMB Admins can also be LOs
  ...
  const handleAssignLead = async () => {
    ...
    const { error } = await supabase
      ...
      .update({ 
        ...
        status: 'Assigned to LO', // Update status upon assignment
        updated_at: new Date().toISOString() 
      })
      .eq('id', lead.id);

    if (error) {
      toast({ title: "Error", description: "Failed to assign lead. " + error.message, variant: "destructive" });
    } else {
      toast({ title: "Success", description: `Lead assigned to ${loanOfficers.find(lo => lo.user_id === selectedLoanOfficerId)?.first_name || 'Loan Officer'}.` });
      onLeadAssigned({ ...lead, assigned_loan_officer_id: selectedLoanOfficerId, status: 'Assigned to LO' });
      onClose();
    }
    setLoading(false);
  };
  ...
  return (
    ...
    >
      ...
      >
        <h2 className="text-2xl font-bold text-gray-900 mb-6">Assign Lead: {lead.buyer_first_name} {lead.buyer_last_name}</h2>
...
export default AdminLeadAssignmentModal;